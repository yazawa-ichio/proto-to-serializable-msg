...
func (m *MyMessage) Pack() ([]byte, error) {
	return protopack.Pack(m)
}

// Write Serialize Message
func (m *MyMessage) Write(w protopack.Writer) error {
	// Write Map Length
...
func (m *MyMessage) Unpack(buf []byte) (error) {
	return protopack.Unpack(m, buf)
}

// Read Deserialize Message
func (m *MyMessage) Read(r protopack.Reader) error {
	// Read Map Length
